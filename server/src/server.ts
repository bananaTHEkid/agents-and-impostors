...
// `CLIENT_ORIGIN` environment variable (comma-separated list). Defaults
// include common dev servers such as Vite (`5173`) and CRA (`3000`) and
// a legacy `5000` entry used previously.
...
// Express CORS middleware using the same allowed origins list. During
// development this will permit connections from Vite/Playwright served pages.
app.use(cors({
    origin: defaultOrigins,
    credentials: true,
}));

// Development-only debug endpoints to inspect current lobbies and players.
